...
To import the MCI_baseline csv, we had to remove the first row since in contained repeat informastion that's in our columns. The dataset lists participants in an Alzheimer's study which goal was to identify if particpants are carriers of the APOE4 gene, which is associated with increased risk of developing alzheimers. The MCI baseline study dataset shows t `r nrow(baseline_df)` participants and `r ncol(baseline_df)` variables. The education variable details 
